...
if __name__ == "__main__":
    ...
    if args.mode=='IDF':
        ...
        files.flatMapValues(lambda s:s.split())\
             ...
             .map(lambda x: (x[1],1))\
             .reduceByKey(lambda x, y: x+y)\
             .mapValues(lambda x:np.log(corpusSize/(1.*x + 1)))\
             .filter(lambda x: x[0]!="")\
             .saveAsTextFile(args.output)
    ...
    if args.mode=='TFIDF':
        ...
        tfidf=tf.join(idf).mapValues(lambda x: x[0]*x[1])
    ...
    if args.mode=='SIM':
        ...
        product = scores1.join(scores2).mapValues(lambda x: x[0]*x[1]).values().reduce(lambda x, y: x+y)
